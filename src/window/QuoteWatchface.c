...
static void update_quote ( ) {
  ...
  for ( int i = 0; i <= (int) ( sizeof ( font_handles ) / sizeof ( font_handles[0] ) ); i++ ) {
    ...
    if ( text_layer_get_content_size ( s_quote_layer ).h < bounds.size.h - BAR_DATETIME_HEIGTH - BAR_AUTHOR ) {
      APP_LOG( APP_LOG_LEVEL_INFO, "Font position: %d, Quote height: %d, Quote max: %d", i, text_layer_get_content_size ( s_quote_layer ).h, bounds.size.h - BAR_DATETIME_HEIGTH - BAR_AUTHOR );
      break;
    }
  }
